// Shared between app/api/jobs/route.ts (client-side search/filter requests)
// and app/jobs/page.tsx (Server Component, fetches the default/unfiltered
// list so real job listings ship in server HTML instead of only appearing
// after a client-side useEffect fetch — see the identical rationale in
// lib/firebase-admin.ts's getDynamicPricing()).
...
export async function fetchJobs(query: string, location: string): Promise<{ jobs: Job[]; source: 'adzuna' | 'static' }> {
  ...
  if (appId && appKey) {
    try {
      ...
      const jobs: Job[] = (json.results ?? []).map((j: AdzunaJob) => ({
        id: j.id,
        title: j.title,
  ...
  const filtered = STATIC_JOBS.filter(j => {
    ...
    const lMatch = !loc || locHay.includes(loc);
    return qMatch && lMatch;
  });

  return { jobs: filtered, source: 'static' };
}
