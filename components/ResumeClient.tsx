...
export default function ResumeClient() {
  ...
  useEffect(() => {
    if (!user) return;
    // Dynamic-imported, not a static top-of-file import — see the identical
    // fix + rationale in components/PricingClient.tsx (a static import of
    // lib/firebase's `db` evaluates at module load time, which happens
    // during SSR too; that module re-throws on init failure, and a throw
    // there silently blanked this component's entire server render).
    (async () => {
