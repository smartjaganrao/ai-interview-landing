...
export default function JobsClient({ initialJobs }: JobsClientProps) {
  ...
  // Seeded from the server-fetched default list (see app/jobs/page.tsx) so
  // real job listings are already in the initial HTML — a plain useState([])
  // here meant every listing was invisible to any crawler that can't run JS.
  ...
  const fetchJobsClient = useCallback(async (q: string, l: string) => {
    setLoading(true);
    try {
          ...
          <div className="flex gap-2 flex-wrap mt-3">
            {ROLES.filter(Boolean).map(r => (
              <button
                key={r}
                type="button"
                onClick={() => { setQuery(r); fetchJobsClient(r, loc); }}
