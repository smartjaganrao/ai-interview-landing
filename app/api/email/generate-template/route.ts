    ...
    for (let i = 0; i < attempts; i++) {
      try {
        ...
        if (isModelNotFoundError(err)) {
          console.warn(`[email-template] Model "${modelCandidate}" deprecation/not found error, falling back...`);
          break; // move to next model candidate
        }
        if (!isJsonGenerationError(err)) throw err;
        console.warn(`[email-template] JSON generation failed on model "${modelCandidate}", retry ${i + 1}/${attempts}...`);
      }
    }
  }
  throw lastErr;
